		...
		<el-container class="home-container">
			<el-aside width="200px">
				...
				<div class="right">
					...
					<div class="son-menu">
						<ul>
							<li v-for="(item, index) in sonMenuList" :key="item.idx" @click="getGrandsonMentList(item, index)">
								<a href="javascript:;" ref="sonMenu" :class="index == 0 ? 'active' : ''">{{item.menuName}}</a>
							</li>
						</ul>
					</div>
				</div>
    ...
    <span>{{account}}</span>
						...
						<ul>
							<li v-for="(item, index) in grandsonMenuLisy" :key="index" @click="getPages(item, index)">
								<a href="javascipt:;" ref="grandsonMenu" :class="index == 0 ? 'active' : ''">{{item.menuName}}</a>
							</li>
						</ul>
					</div>
				</el-header>
