...
<!--      </el-form-item>-->
      ...
      <el-form-item>
        ...
        <el-checkbox v-if="false" v-model="awae.hasPdMaxOut" @change="resetPdMaxOut">每天出奖总次数限制</el-checkbox>
...
<script>
export default {
  props: ['awae', 'prizeType', 'nWin', 'isRed', 'hide', 'astrict','configId'],
  data() {
  ...
  methods: {
    ...
    addRepertory() {
      this.$prompt('请输入数字', '增库', {
          ...
          this.$request.post('/api/flower/prize/add', {
            flowerConfigId : this.configId,
            prizeId : this.awae.id,
            num : value
          }, false, res => {
            if (res.code == '200') {
