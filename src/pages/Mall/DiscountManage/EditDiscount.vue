  ...
  <div class="container">
    <el-card v-loading="loading">
      <el-col class="mb20">
        ...
        <el-form :model="ruleForm" :rules="rules" :disabled="disable == 1" ref="ruleForm" label-width="120px" class="demo-ruleForm">
        ...
        <template v-for="(item,index) in discountConf">
          <el-form ref="discountConf" :model="item" :disabled="disable == 1" label-width="150px" :rules="rules">
            <el-card :body-style="{ padding: '20px' }">
              <div slot="header" class="clearfix">
                <span>折扣信息{{index+1}}：</span>
                <span v-if="disable == 0" style="float: right;color: red;cursor: pointer" @click="delteDiscountItem(index)">删除</span>
              </div>
              ...
              <el-tabs v-model="item.listType" type="card" @tab-click="handleClickTab">
                <el-tab-pane label="自建商品" name="ZJ">
                  <el-table
                    :data="item.zj"
                    style="width: 100%">
                    <el-table-column property="productId" label="商品ID" width="200">
                      <template slot-scope="scope">

                        <span v-if="!isNaN(scope.row.productId)">{{ scope.row.productId }}</span>
                    ...
                    <el-table-column property="firstCategoryName" label="一级分类">
                      <template slot-scope="scope">
                        ...
                        <span v-else>{{ scope.row.cate1Name }}</span>
                      </template>
                    </el-table-column>
                    <el-table-column property="secondCategoryName" label="二级分类"></el-table-column>
                    <el-table-column prop="score" label="兑换值"></el-table-column>
                    <el-table-column prop="discountScore" label="折扣价">
                      <template slot-scope="scope">
                        <span v-if="scope.row.discountScore">{{ scope.row.discountScore }}</span>
                        <span v-else>{{ parseFloat((scope.row.score * item.discount).toPrecision(12))}}</span>
                      </template>
                    </el-table-column>
                    <el-table-column prop="shopQuantity" label="商品库存"></el-table-column>
                    <el-table-column prop="shopQuantity" label="可兑换库存">
                      <template slot-scope="scope">
                        <span v-if="scope.row.total_num">{{ scope.row.total_num - scope.row.outNum }}</span>
                        <span v-else>{{ item.num }}</span>
                      </template>
                    </el-table-column>
                    <el-table-column prop="productStatus" label="商品类型">
                      <template slot-scope="scope">
                        <span>{{ scope.row.productStatus == "1" ? "上架中":scope.row.productStatus == "-1" ? "下架" : "已停用"}}</span>
                      </template>
                    </el-table-column>
                    <el-table-column
                        fixed="right"
                  ...
                  <el-table
                    ...
                    <el-table-column property="productId" label="商品ID" width="200">
                      <template slot-scope="scope">
                        ...
                        <span v-if="!isNaN(scope.row.productId)&&scope.row.productId<10000">{{ scope.row.productId }}</span>
                        <span v-else>{{ scope.row.id }}</span>
                      </template>
                    </el-table-column>
                    <el-table-column property="memo" label="商品名称" width="200"></el-table-column>
      ...
      <el-button v-if="disable == 0" type="primary" @click="save">保存</el-button>
      <el-button @click="returnDiscount">返回</el-button>
    </el-card>
...
//分类列表
    ...
    methods: {
      ...
      getDetail(){
        this.$request.post('/sc/discount/act/select/one', {id:this.id}, false, (res) => {
          if (res.code == '200') {
            this.ruleForm.name = res.data.actInfo.actName
            this.ruleForm.stime = res.data.actInfo.stime
            this.ruleForm.etime = res.data.actInfo.etime
          }
          this.discountConf = res.data.itemList.map((item) => {
            return {
              id: item.id,
              actId: item.actId,
              ...
              listType: 'ZJ',
              zj:res.data[item.id + "_self"],
              sn:res.data[item.id + "_sn"],
            }
          })
        })
      },
            ...
            if(this.discountConf[this.discountIndex].sn.length == 0) {
              ...
              this.selectProductJD.forEach((item,index) => {
                let i = this.discountConf[this.discountIndex].sn.findIndex((i)=>{
                  return i.productId == item.id || i.productId == item.productId
                })
                if(i==-1){
          ...
          if (valid) {
            ...
            let actItemWrapperList = this.discountConf.map((item)=>{
              let zj = item.zj.map((i)=>{    //商品类型 1 自建 4. 苏宁
                if(!isNaN(i.productId)){
                  return {
                    "id":i.id,
                    "productId":i.productId, //商品id . 商品信息里的id字段.
                    "productType":1
                  }
                }else {
                  return {
              ...
              let sn = item.sn.map((i)=>{
                if(!isNaN(i.productId)&&i.productId<10000){
                  return {
                    "id":i.id,
                    "productId":i.productId, //商品id . 商品信息里的id字段.
                    "productType":4
            ...
            console.log(params)
            this.$request.post('/sc/discount/act/update', params, true, res => {
              if (res.code == '200') {
