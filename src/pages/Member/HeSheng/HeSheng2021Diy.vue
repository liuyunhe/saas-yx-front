<template>
  <div class="container">
    <el-card v-loading="loading">
      ...
      <el-col class="mb20">
        <div>当前已有<span style="color: #409EFF">{{ openawdUserNum }}</span>人获得瓜分大奖资格</div>
      </el-col>
      <el-col class="mb20">
        <el-card :body-style="{ padding: '20px' }">
          ...
          <el-form :model="info" :rules="rules" ref="ruleForm" label-width="200px" class="demo-ruleForm">
            <el-form-item label="DIY活动ID：" prop="actCode">
              <el-input :disabled="!!info.id" v-model="info.actCode" style="width: 220px"></el-input>
            </el-form-item>
            <el-form-item label="DIY活动结束参与时间：" prop="joinEtime">
...
<script>
  export default {
    ...
    methods: {
      getInfo() {
        this.$request.post('/hbact/saas/diy/configInfo', {}, true, res => {
          this.loading = false
          if (res.code == '200') {
      ...
      save(){
        ...
        this.$request.post('/hbact/saas/diy/configUpdate', params, true, res => {
          this.loading = false
          if (res.code == '200') {
      ...
      open(){
        this.$request.post('/hbact/saas/diy/awardOpen', {configId:this.info.id}, false, res => {
          if (res.code == '200') {
