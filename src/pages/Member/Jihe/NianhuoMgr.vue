...
<!--            </el-form-item>-->
            ...
            <el-form-item label='奖品数量：'  v-if="!!ai.id">
              <span  style="margin-left: 20px">剩余：{{ ai.numTotal - ai.numUsed }}</span>
            </el-form-item>
            <el-form-item  label='名次起始值：' prop="rankStart">
              <el-input-number :disabled="!!ai.id" :step="1" :precision="0":min="0"  controls-position="right" v-model="ai.rankStart"></el-input-number>
              <span></span>
            </el-form-item>
            <el-form-item  label='名次结束值：' prop="rankEnd">
              <el-input-number :disabled="!!ai.id" :step="1" :precision="0":min="0"  controls-position="right" v-model="ai.rankEnd"></el-input-number>
              <span v-if="ai.rankStart>ai.rankEnd" style="color: red">名次起始值不能大于名次结束值</span>
            </el-form-item>
  ...
  export default {
    ...
    data(){
      ...
      return {
        ...
        confRules: {
          ...
          rankStart: [{ required: true, message: '请输入名次起始值',trigger:'blur' }],
          rankEnd: [{ required: true, message: '请输入名次结束值',trigger:'blur' }],
        },
    ...
    methods:{
      ...
      getAwardDetail(actCode) {
        this.$request.post('/hbact/saas/zt/ht/xunbao/queryConfig', {actCode}, false, res => {
          if (res.code == '200') {
            this.newAct = false
      ...
      confirmSubmit() {
        ...
        console.log(params)
        this.$request.post('/hbact/saas/zt/ht/xunbao/awardConfig', params, true, res => {
          if (res.code == '200') {
