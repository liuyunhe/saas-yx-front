    ...
    <el-col class="crm-table-wrap" v-loading="listLoading" :span="24">
      <table class="crm-table" width="100%">
        ...
        <tr>
          ...
          <th style="width: 50px"><div>姓名</div></th>
          <th style="width: 50px">省份</th>
          <th style="width: 80px">市区</th>
          <th style="width: 80px">地址</th>
          <th style="width: 80px">邮编</th>
          <th class="ctrl" style="width: 200px">操作</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="item in pageList">
          <td :title="istitle(item.date)" style="width: 300px">{{item.date}}</td>
          <td :title="istitle(item.name)">{{item.name}}</td>
          <td :title="istitle(item.province)">{{item.province}}</td>
          <td :title="istitle(item.cityName)">{{ item.city }}</td>
          <td :title="istitle(item.address)">{{ item.address }}</td>
          <td :title="istitle(item.zip)">{{ item.zip }}</td>
          <td>
            <span class="ctrl" @click="previewLandingPage(item)">预览</span>
            <!--<span class="ctrl" v-if="filters.status==='0'">检测</span>-->
            <span class="ctrl" @click="editLandingpage(item)">编辑</span>
