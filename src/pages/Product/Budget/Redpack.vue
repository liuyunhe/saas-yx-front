    ...
    <div>
        ...
        <el-card>
            ...
            <el-table :data="tableList" style="width: 100%">
                ...
                <el-table-column prop="budgetValue" label="当季度预算（元）" align="center" >
                    <template slot-scope="scope">
                        <span>{{scope.row.budgetValue}}元</span>
                    </template>
                </el-table-column>
                <el-table-column prop="statGiven" label="已发放（元）" align="center" >
                    <template slot-scope="scope">
                        <span>{{scope.row.statGiven || 0}}元</span>
                    </template>
                </el-table-column>
                <el-table-column prop="statReceived" label="已领取（元）" align="center" >
                    <template slot-scope="scope">
                ...
                <el-table-column prop="statRest" label="季度结算(元)" align="center" >
                    <template slot-scope="scope">
                        <span>{{scope.row.statRest || 0}}元</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="新建红包" width="500px" :visible.sync="form.show" @close="resetForm()">
            <el-form ref="ruleForm" label-width="150px" :model="form" :rules="formRules">
                <el-form-item label="红包礼品名称：" prop="materialName">
                    <el-button size="small" @click="getList(3)">{{ form.materialName ? form.materialName : "选择" }}</el-button>
                    <el-input style="display: none;" size="small" v-model="form.materialName" placeholder="请输入礼品名称"></el-input>
                </el-form-item>
                <el-form-item label="剩余库存（元）：" prop="stock">
                    <el-input-number v-model="form.stock" :precision="0" :min="0" controls-position="right" disabled></el-input-number>
                </el-form-item>
                <el-form-item label="预算（元）：" prop="budgetValue">
                    <el-input-number v-model="form.budgetValue" :precision="0" :min="0" :max="form.stock || 0" controls-position="right"></el-input-number>
...
<script>
    export default {
        ...
        methods: {
            ...
            addRepertory(item) {
                this.$request.post('/api/wiseqr/metra/detail', {id:item.materialPoolId, metraFlag:'redpack'}, true, (res)=>{
                    if (res.ret == '200000') {
                        let stock = res.data.stock
                        this.$prompt(
                            `<div style="line-height: 30px">物料库红包池：${item.materialName}</div>
                         <div style="line-height: 30px">剩余库存（元）：${ stock }</div>
                         增加预算（元）：（提示：不可超过剩余库存）`,
                            '增库', {
